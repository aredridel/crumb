...
const logger = require('bole')('@npmcorp/crumb');
...
exports.register = function (server, options, next) {
    ...
    server.ext('onPostAuth', (request, reply) => {
        ...
        if (settings.restful === false ||
            ...
            if (content[request.route.settings.plugins._crumb.key] !== request.plugins.crumb) {
                logger(request.id).info("crumb mismatch: content %j key %j crumb %j", content, request.route.settings.plugins._crumb.key, request.plugins.crumb)
                return reply(Boom.forbidden());
            }
    ...
    const generate = function (request, reply) {
        ...
        if (!crumb) {
            ...
            logger(request.id).info('New crumb "%s" for request state %j', crumb, request.state);
        }
...
exports.register.attributes = {
    pkg: Object.assign(require('../package.json'), { name: 'crumb' })
};
